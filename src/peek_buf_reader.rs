...
use async_std::io::{self, BufRead, Read, Seek, SeekFrom};
...
pin_project! {
    ...
    /// ```
    ///
    /// This struct is copied wholesale from async_std with minor tweaks.
...
impl<R: io::Read> BufReader<R> {
    ...
    /// Create a new buffered reader with some data pre-buffered.
    pub fn with_inital_buffer(initial: &[u8], capacity: usize, inner: R) -> BufReader<R> {
        let cap = capacity.max(initial.len());
